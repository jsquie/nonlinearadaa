...
double Li2(double x) noexcept {
  ...
  const double P[] = {0.9999999999999999502e+0, -2.6883926818565423430e+0,
                      2.6477222699473109692e+0, -1.1538559607887416355e+0,
                      2.0886077795020607837e-1, -1.0859777134152463084e-2};
  const double Q[] = {1.0000000000000000000e+0, -2.9383926818565635485e+0,
...
/**
 * @brief Real dilogarithm \f$\operatorname{Li}_2(z)\f$ with long double
 * precision
 * @param x real argument
...
long double Li2(long double x) noexcept {
...
#if LDBL_DIG <= 18
  const long double P[] = {1.07061055633093042767673531395124630e+0L,
  ...
  const long double Q[] = {1.00000000000000000000000000000000000e+0L,
...
#else
  const long double P[] = {1.0706105563309304276767353139512463033e+0L,
  ...
  const long double Q[] = {1.0000000000000000000000000000000000000e+0L,
...
std::complex<float> Li2(const std::complex<float>& z_) noexcept {
  ...
  const float bf[] = {-1.0f / 4, +1.0f / 36, -1.0f / 3600, +1.0f / 211680};
...
/**
 * @brief Complex dilogarithm \f$\operatorname{Li}_2(z)\f$ with long double
 * precision
 * @param z_ complex argument
 * @return \f$\operatorname{Li}_2(z)\f$
 * @note Implementation translated from SPheno to C++
 * @author Werner Porod
 * @note translated to C++ and extended to long double precision by Alexander
 * Voigt
 */
std::complex<long double> Li2(const std::complex<long double>& z_) noexcept {
  ...
  const long double bf[] = {-1.0L / 4.0L,
                            1.0L / 36.0L,
                            -1.0L / 3600.0L,
                            1.0L / 211680.0L,
                            -1.0L / 10886400.0L,
